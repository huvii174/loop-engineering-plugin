...
37 patterns that waste tokens and cause re-prompts. Read this file when
compiling a goal into `.loop/prompt.md`, when a user pastes a prompt to fix,
or when diagnosing an iteration that produced the wrong thing from a correct
plan.

Adapted from `prompt-master` v1.7.0 by Nidhin Joseph Nelson (MIT):
...
## Which loop gate catches which pattern

The loop already has gates for most of these. The point of the list is to catch
them at compile time, one round of questions, instead of at iteration 4.

| Patterns | Caught by | Caught when |
|---|---|---|
| 1, 3, 5, 14, 15 (vague verb, no criteria, emotional description, no format, implicit length) | design gate, success-criteria dimension | before any code runs |
| 2, 6, 25 (two tasks, build-the-whole-thing, whole codebase as context) | `/loop-engineering:breakdown`, tier routing | at epic split |
| 7, 8, 9, 10, 13, 28, 29 (implicit reference, assumed knowledge, forgotten stack, contradicted prior work) | `.loop/memory/` recall plus the carry-forward block | at design gate step 0 |
| 4, 20, 22, 23, 34, 35 (over-permissive agent, no scope, no stop condition, unlocked filesystem) | `Must not:` lines, global boundaries, `boundary-gate` hook | at edit time, deterministically |
| 11, 30 (hallucination invite, no grounding rule) | external anchors in `Done when:` | at verification |
| 31, 32, 33 (no starting state, no target state, silent agent) | Template N brief contract | at every `Agent()` spawn |
| 36, 37 (vague first turn on Opus, context rot) | `.loop/prompt.md` compilation, per-iteration scoping | at the 95% gate |

Patterns 16 to 19, 21, 24, 26, 27 stay the responsibility of whoever writes the
prompt, which inside the loop means this skill. Nothing downstream checks them.

The residue is the part worth watching: a pattern with no gate behind it fails
silently, and the loop reads the resulting narrow output as a correct answer to
