...
Per-tool prompting rules for every target the loop might have to write a prompt
for. Read only the section you need; never load the whole file to write one
prompt.

Inside `/loop-engineering:*` the target is almost always **Claude Code** (this
session, or one of its subagents), so start at that section. The rest of the
file matters when the *goal itself* is to produce a prompt for another tool:
a Cursor rule file, a Midjourney string, an n8n step, an ElevenLabs script.
...
## Loop-local routing defaults

When the receiver is this session or one of its subagents, the routing above
collapses to a fixed profile. Use it without re-deriving:

| Receiver | Profile |
|---|---|
| The main loop session (Claude Code, Opus) | Template M brief. Front-load intent, file scope, constraints, acceptance criteria. No thinking-budget instruction. |
| `loop-engineering:loop-verifier` | Template N brief. Criteria quoted verbatim, project root absolute, evidence contract explicit. |
| `loop-engineering:plan-critic` | Template N brief. Inputs are file paths, not pasted files. Charter says refute, not review. |
| `loop-engineering:epic-planner` | Template N brief. Output contract is the backlog table shape, proposal only. |
| Review-gate reviewers (`general-purpose`, `sonnet`/`haiku`) | Template N brief, one dimension charter per agent, read-only clause mandatory. |

Two rules cut across all five: a subagent has fresh context and no way to ask a
follow-up, so anything it needs goes in the first turn; and no brief tells an
Opus agent to think step by step, because adaptive thinking calibrates itself
and the instruction only costs tokens.
