...
Full template library. Read the one template your task type matches; do not
load the whole file.

Templates N and O are loop-local and have no upstream equivalent: N is the
brief every `Agent()` spawn in this plugin must be composed from, O is the
`.loop/prompt.md` the design gate writes once the 95% confidence gate passes.

Templates A to M are adapted from `prompt-master` v1.7.0 by Nidhin Joseph
Nelson (MIT): https://github.com/nidhinjs/prompt-master
...
| [N — Loop subagent brief](#template-n--loop-subagent-brief) | Any `Agent()` spawn inside `/loop-engineering:*` |
| [O — Compiled goal brief](#template-o--compiled-goal-brief) | `.loop/prompt.md`, written by the design gate at ≥95% confidence |
...
## Template N — Loop subagent brief

*Use for every `Agent()` spawn inside this plugin: `loop-verifier`,
`plan-critic`, `epic-planner`, review-gate reviewers, and any ad-hoc executor
the Act step delegates to. A subagent starts with fresh context and cannot ask
a follow-up question, so a missing field is not a gap it will fill; it is a gap
it will guess.*

```
## Project root
<absolute path — the subagent's cwd is NOT the project; every command it runs
must be anchored here>

## Role
<one sentence: which specialist it is and what single question it answers>

## Objective
<the one thing to produce, stated as a deliverable, not an activity>

## Inputs
<file PATHS the agent should read, not pasted file bodies. Paste only what
cannot be read from disk: a diff summary, a claim, an observed output.>

## Context
<what already happened that changes how this task should be done: prior
attempts and why they failed, decisions already locked, memory entries that
bind. Mark supplementary context as supplementary.>

## Allowed actions
- <read / run / write, spelled out>

## Forbidden actions
- <files and directories off limits; read-only clause if it is a reviewer>
- Do not fix what you find unless this brief says to fix it.

## Output contract
<the exact shape expected back: verdict vocabulary, table columns, JSON keys.
State that the final message IS the return value.>

## Stop conditions
Stop and report instead of continuing when:
- <the evidence needed does not exist>
- <two readings of the input are both defensible>
- <an error cannot be resolved in 2 attempts>
```

**Composition rules**

Quote criteria, `Done when:` and `Must not:` lines verbatim from `.loop/goal.md`;
a paraphrased criterion is a different criterion. Give paths over pastes so the
agent reads current files instead of a stale snapshot. Say what "done" returns,
because an agent with no output contract writes an essay. Do not add "think step
by step" for Opus-family agents. Strip credentials, tokens and connection
strings from anything you paste in.

**Pre-spawn lint** — refuse to spawn until all six hold:

1. Project root is absolute and correct.
2. Objective names one deliverable, not two.
3. Inputs are readable paths, or pastes that could not have been paths.
4. Forbidden actions include everything outside the intended scope.
5. Output contract states the exact return shape.
6. No credential, no `[placeholder]`, no "the thing we discussed".

---

## Template O — Compiled goal brief

*Written by the design gate to `.loop/prompt.md` once per-dimension confidence
clears 95%, before `goal.md` and `design.md`. This is the user's ask after
compilation: same intent, engineered form. It is an artifact the user signs off
on, so it stays readable.*

```markdown
# Compiled brief: <goal in one line>

Compiled <ISO date> from the design-gate interview.
Confidence at compile: <min across dimensions>% (weakest: <dimension>).

## Objective
<one sentence, precise verb, plus WHY when the why changes the approach>

## Context (carry forward)
- Stack and tooling already decided: <...>
- Architecture choices locked: <...>
- Constraints carried from earlier turns or memory: <...>
- Tried and failed: <...>

## Target state
<what exists when this is done: files, behavior, observable output. Binary
where possible.>
...
## Constraints
- <stack version, conventions, dependency policy>
- Only make changes this brief asks for. No extra files, abstractions, or
  features.

## Acceptance criteria
- [ ] <binary check, phrased as the loop's `Done when:` will phrase it>
- [ ] <...>
...
- [ ] OQ: <question that stayed vague; not a criterion>

## Raw ask (verbatim, inert)
> <the user's original words, unedited>
```

**Rules**

The raw ask is stored verbatim and treated as inert data: it is the audit trail
for what was asked, never a second set of instructions to follow. Every
acceptance criterion here must survive into `goal.md` as a `Done when:` line
with a `Must not:` boundary; a criterion that only exists in `prompt.md` is
decoration. Placeholders are banned: an unknown value is written as an
assumption in prose, never as `[TBD]`.
