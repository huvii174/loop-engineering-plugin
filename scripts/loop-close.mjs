 ...
 *   node loop-close.mjs close --item <n> --verdict-file <path> [--dir .loop]
 *
...
const FLAGS = { item: 'value', dir: 'value', 'verdict-file': 'value', help: 'bool' };
...
/** Criterion blocks (`- [ ] C2b …`) with their one `Done when:`. */
function criteriaOf(file, reg) {
  ...
  return blocksOf(file, lines,
    ...
    if (next !== undefined && CONT.test(next) && !/^\s*(Evidence|Sites|Must not):/.test(next)) {
    ...
    return { id: c.id, no: c.no, block: c.block, doneWhen: dones.length ? dones[0][1].trim() : null };
  });
}
...
function readProven(file, text) {
  ...
  heads.forEach((h, k) => {
    ...
    items.set(n, criteriaOf(file, rest));
    at_.set(n, h.no);
  });
...
function load(dir, item) {
  ...
  for (const up of backlog.rows.filter((r) => r.n !== item && isDone(backlogFile, r))) {
    ...
    for (const c of crit) {
      ...
      list.push({ id: `item-${up.n}/${c.id}`, text: `Done when: ${c.doneWhen}`, doneWhen: c.doneWhen, file: provenFile, no: c.no });
    }
  }
  for (const c of own) {
    if (!c.doneWhen) refuse(`goal.md criterion ${c.id} has no \`Done when:\` line.`);
    list.push({ id: `item-${item}/${c.id}`, text: `Done when: ${c.doneWhen}`, doneWhen: c.doneWhen, file: goalFile, no: c.no });
  }
  ...
  return { slug, base, backlog, row, provenText, goal, own, list };
}
...
function renderPlan(ctx) {
  const out = [
    ...
    '',
    'Re-run every criterion below. Report one line per id: `- <id>: met` or',
    '`- <id>: not met — <reason>` (a `Criterion: "<Done when>" → met` line in the',
    'loop-verifier\'s own shape counts for the criterion it quotes). Name an id on',
    'its own line only: any other line that mentions it blocks the close.',
  ];
...
/** Per entry: null (never named), 'met', or the first naming line that was not its met statement; plus the not-met lines that name no id. */
function marks(text, list) {
  ...
  for (const raw of text.split('\n')) {
    ...
    if (UNMET.test(scan)) stray.push(line.trim());
  }
  return { state, stray };
}

function checkVerdict(text, list) {
  ...
  const { state, stray } = marks(text, list);
  ...
  if (stray.length) refuse(`the verdict says not met on a line naming no listed id: "${stray[0]}" — name the id on that line.`);
  if (missing.length) {
    refuse(`the verdict never marks ${missing.join(', ')} met — every listed id needs a \`- <id>: met\` line ` +
...
function main(argv) {
  ...
  try {
    ...
    const verdictLine = `closed: APPROVE (${date}; ${ids}; ${args['verdict-file']})`;
